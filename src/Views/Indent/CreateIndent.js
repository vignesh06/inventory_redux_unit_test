...
function CreateIndent(props) {
  const [indentform, setindentform] = useState({ type: '', location_id: '', deliveryDate: '', indentLineList: [] });
  ...
  const AddProducttoIndentList = () => {
    let formObject = Object.assign({}, indentform);
    formObject.indentLineList.push(productindentform)
    setindentform({ ...indentform, ...formObject })
    setproductindentform({ product_id: '', quantity: '', unitPrice: '' })
  }
  const CreateIndent = async () => {
    let url = UrlConstant.Ip + UrlConstant.Indent.indent
    let formObject = Object.assign({}, indentform);
    formObject['deliveryDate'] = '11/12/2019';
    let data = await HTTPService(url, 'POST', formObject)
  ...
  return (
    <React.Fragment>
      <Container>
        <PageHeading heading={'Create Indent'} />
        <CreateorUpdateIndent
          indentform={indentform}
          seIndentFormData={(e, field) => seIndentFormData(e, field)}
        />
        <AddProducttoIndent productindentform={productindentform} setProductIndentFormData={(e, field) => setProductIndentFormData(e, field)} AddProducttoIndentList={() => AddProducttoIndentList()} />
        <Row>
        ...
        <Row>
          <Col md={6} lg={6} sm={6} xs={6}>
            <br />
            <Button variant="primary" onClick={CreateIndent} size="md" disabled={!(indentform.type && indentform.location_id && indentform.deliveryDate && indentform.indentLineList.length > 0)}>Create</Button>&nbsp;&nbsp;&nbsp;
